...
function _onAuthorize() {
  ...
  SpreadsheetApp.getUi().showModalDialog(
    HtmlService.createHtmlOutput(`<script>function authorize() { window.open("${authorizationUrl}", '_blank').focus(); google.script.host.close(); }</script><button onclick="authorize()">Authorize</button>`),
    "Authorize"
  )
}
...
function getChannels() {
  ...
  const privateChannels = IPDX.getPaginated("conversations.list", "types", "public_channel", "exclude_archived", "true");
...
function getChannelsWithLastMessageTimestamp() {
  ...
  const publicChannels = IPDX.getPaginated("conversations.list", "types", "public_channel", "exclude_archived", "true");
  const privateChannels = IPDX.getPaginated("conversations.list", "types", "public_channel", "exclude_archived", "true");
  let channels = [...publicChannels, ...privateChannels];  channels = flatMap(channels, "channels");
  channels = mapReduce(channels, "id", "name", "is_private", "num_members", "is_member");
